...
removed_rows = (
  df['Title'].isna() |
  df['ID'].isin([3054, 5307, 5308, 5309, 5310, 5312, 5314, 5315, 5318, 5319, 5320, 5321, 5322, 5323, 5324, 5325, 268, 1284, 2459, 2675, 3531, 3716, 3889, 3890, 3891, 4572]) |
  (
    df['Owner Type'].isna() &
    df['Owner ID'].isna() &
...
df.loc[~removed_rows].to_csv(dataset_path + '/lessons.preprocessed.csv', index=False, encoding='utf-8')

df = df.loc[removed_rows]
df.to_csv(dataset_path + '/lessons.removed.csv', index=False, encoding='utf-8')
removed_lesson_ids = df['ID'].array
